...
> 当前版本 v1.3.0。版本节奏遵循全局约定：常规迭代 +0.0.1，大功能升级 +0.1.0。
...
### Phase 0 · 云端修复（✅ 已完成，2026-09-19）

把已写好的 P1 后端真正点亮，是后续社交/同步类功能的地基。

- [x] 建 D1 库并解 `database not found`（database_id: `2e3abcd8…`，已写入 wrangler.toml）
- [x] 解除 `wrangler.toml` 中 `[[d1_databases]]` 注释并填真实 `database_id`（7a599a7）
- [ ] 端到端验收：手机注册 → 另一浏览器登录，进度 / 生词本 / 社区帖无缝衔接（待领主部署后验收）
- [ ] 离线队列在真实网络抖动下的重试与冲突处理加固（观察线上后再定）
- **部署 / 建库动作由领主手动跑**（咪咪只准备代码 + 本地验证 + commit/push）
...
- 保留零依赖零构建；Phase 1 内容外置是减轻单文件膨胀（现 1466 行 / 104KB）的关键一步。
- 若功能持续膨胀，再考虑「壳 `index.html` + 模块化原生 JS」，仍不引入构建链。
- 版本节奏：常规迭代 +0.0.1，大功能升级 +0.1.0（遵循全局约定）。

---

## 下一步执行计划（2026-09-19 定稿）

> Phase 0 已点亮，D1 可用 → 社交/同步/AI 类功能的地基就绪。执行顺序按下述节奏，每个版本一个主题。

### 立即收尾（不等版本，本周内）

- [x] `git push` 落库云端点亮提交（f4dea40，2026-09-19）
- [ ] 领主手动部署后，做跨设备 E2E 验收（手机注册 → 电脑登录，进度/生词本/社区帖衔接）——Phase 0 的最后两颗星
- [ ] 接入 Cloudflare Web Analytics（一行脚本，先看真实留存再定优先级）

### v1.3.0 · 内容工程 + 轻体验（✅ 已完成，2026-09-19）

单文件已 1466 行 / 104KB，内容不外置就无法持续扩课，这是当前最大的结构性瓶颈；顺手带两个低依赖体验项。

- [x] 课程内容外置：新增 B2/C1 及扩展单元放 `data/{en,ja,ko}.json`，按需 fetch + SW 预缓存（network-first 支持热更）；`file://` 直开自动降级内嵌 A1→B1 课程
- [x] 补齐 B2/C1 单元，解锁完整 A1→C1 六级（每语种 B2×2 + C1×1，锁定状态由「有无内容」动态判定）
- [x] GitHub 风格学习热力图（进度页年度打卡日历，基于 u.days 本地数据）
- [x] 键盘快捷键：flashcard 空格翻转 / 数字键选答案 / F 发音 / ←→ 切卡片（学习页+复习页）

### v1.4.0 · 听力 & 口语进阶

- [ ] 听力：0.5×~1.5× 变速、单句精听（逐句 AB 循环）、听写模式（逐句填空判分）
- [ ] 口语：SpeechRecognition 降级自评流打磨（iOS Safari / Firefox）
- [ ] 拼写测试模式（听音拼词 / 打乱字母还原，接入生词本队列）

### v1.5.0 · 游戏化 & 社交深化（依赖已就绪）

- [ ] 赛季排行榜、段位、连胜保护（streak freeze）
- [ ] 学习小组 / 好友挑战 / 7 日打卡挑战赛
- [ ] 社区升级：话题标签、@提醒
- [ ] 每日目标 Web Push 提醒（需后端，地基已具备）

### 穿插项 · AI 能力支柱（差异化，v1.4~v1.5 期间见缝插针）
