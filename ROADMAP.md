...
- [ ] `git push` 落库当前 7 个提交（云端点亮代码）
- [ ] 领主手动部署后，做跨设备 E2E 验收（手机注册 → 电脑登录，进度/生词本/社区帖衔接）——Phase 0 的最后两颗星
- [ ] 接入 Cloudflare Web Analytics（一行脚本，先看真实留存再定优先级）

### v1.3.0 · 内容工程 + 轻体验（下一版本，推荐起点）

单文件已 1466 行 / 104KB，内容不外置就无法持续扩课，这是当前最大的结构性瓶颈；顺手带两个低依赖体验项。

- [ ] 课程内容外置 `data/{en,ja,ko}.json`，按需 fetch + SW 预缓存（index.html 只留骨架与引擎）
- [ ] 补齐 B2/C1 单元，解锁完整 A1→C1 六级；每级扩到 4+ 单元，新增场景（购物、就医、面试、租房）
- [ ] GitHub 风格学习热力图（打卡日历可视化，纯本地数据，强化连续打卡动力）
- [ ] 键盘快捷键：flashcard 空格翻转 / 数字键选答案 / F 播发音（桌面端学习效率）
